...
def main():
    # 1. Wczytanie modelu do embeddingów
    ...
    # 2. Sprawdzenie czy model istnieje
    if model_path.exists():
        print(f"Znaleziono zapisany model: {model_path}. Pomijam trenowanie...")
        model = joblib.load(model_path)
    else:
        print("Brak zapisanego modelu. Inicjalizacja LightGBM...")
        ...
        print("Trenowanie LightGBM na wyekstrahowanych cechach...")
        model.fit(X_train, y_train)

        print(f"Zapisuję wytrenowany model do pliku {model_path}...")
